<?php include 'inc/header.php'; ?>

<!-- Hero Banner Section Start -->
<div class="hero-banner">
    <img src="assets/images/products-banner.jpg" alt="about us banner image">
    <h3><span data-aos="fade-up">Products</span></h3>
    <div class="bg"></div>
</div>
<!-- Hero Banner Section End -->

<!-- Breadcrumb Section Start -->
<div class="breadcrumb-wrapper bg-white">
    <div class="breadcrumb-inner center" data-aos="fade-up">
        <div class="nav">
            <p>Location：</p>
            <a href="index.php">Home</a>
            <span>I</span>
            <a href="about-us.php">Products</a>
        </div>
        <div class="right">
            <a href="products.php">Dishwashers</a>
            <a href="fan-motor.php" class="active">Fan Motor</a>
            <a href="dc-fan.php">DC Fan</a>
            <a href="bldc.php">BLDC</a>
        </div>
    </div>
</div>
<!-- Breadcrumb Section End -->

<!-- Product List Section Start -->
<div class="product-list-wrapper fan-motor-products">
    <div class="product-list-content center">
        <!-- product list -->
        <ul class="product-lists" data-aos="fade-up">
            <!-- product item -->
            <li class="product-item">
                <a href="../shows/5/1-2.html">
                    <img src="assets/images/product-list-image-10.jpg" alt="product list image">
                    <div class="ct">
                        <h3>YYK9-2B 9inch Stormy Fan Motor</h3>
                        <h6>9inch Stormy Fan Motor</h6>
                    </div>
                </a>
            </li>
            <!-- product item -->
            <li class="product-item">
                <a href="../shows/5/2.html">
                    <img src="assets/images/product-list-image-11.jpg" alt="product list image">
                    <div class="ct">
                        <h3>YYK12-4A Table Fan Motor</h3>
                        <h6>Table Fan Motor</h6>
                    </div>
                </a>
            </li>
            <!-- product item -->
            <li class="product-item">
                <a href="../shows/5/3.html">
                    <img src="assets/images/product-list-image-12.jpg" alt="product list image">
                    <div class="ct">
                        <h3>YYK12-4B Table Fan Motor</h3>
                        <h6>Table Fan Motor</h6>
                    </div>
                </a>
            </li>
            <!-- product item -->
            <li class="product-item">
                <a href="../shows/5/4.html">
                    <img src="assets/images/product-list-image-13.jpg" alt="product list image">
                    <div class="ct">
                        <h3>YYYK16-4A Stand Fan Motor</h3>
                        <h6>Stand Fan Motor</h6>
                    </div>
                </a>
            </li>
            <!-- product item -->
            <li class="product-item">
                <a href="../shows/5/5.html">
                    <img src="assets/images/product-list-image-14.jpg" alt="product list image">
                    <div class="ct">
                        <h3>YYK16-4J Net Fan Motor</h3>
                        <h6>Stand Fan Motor</h6>
                    </div>
                </a>
            </li>
            <!-- product item -->
            <li class="product-item">
                <a href="../shows/5/6.html">
                    <img src="assets/images/product-list-image-15.png" alt="product list image">
                    <div class="ct">
                        <h3>YYK16-4C Stand Fan Motor</h3>
                        <h6>Net Fan Motor</h6>
                    </div>
                </a>
            </li>
            <!-- product item -->
            <li class="product-item">
                <a href="../shows/5/7.html">
                    <img src="assets/images/product-list-image-16.png" alt="product list image">
                    <div class="ct">
                        <h3>YYK16-2B High speed Stand Fan Motor</h3>
                        <h6>High speed Stand Fan Motor</h6>
                    </div>
                </a>
            </li>
        </ul>
        <!-- pagination -->
        <div class="product-list-pagination" data-aos="fade-up">
            <div class="el-pagination">
                <button type="button" disabled="disabled" class="btn-prev">first</button>
                <ul class="el-pager">
                    <li class="number active">1</li>
                </ul>
                <button type="button" class="btn-next">last</button>
            </div>
        </div>
    </div>
</div>
<!-- Product List Section End -->
<?php include 'inc/footer.php'; ?>